...
const EntryCard: React.FC<IDownloadModalProps> = (props) => {
  ...
  return (
    <div className={`h-screen w-screen bg-black bg-opacity-70 flex items-center justify-center ${ props.open ? 'fixed' : 'hidden' }`}>
      <div className="bg-white mx-4 px-6 py-10 rounded-2xl flex flex-col gap-4 items-center md:flex-row md:justify-between md:px-20 relative"> 
        <span className="absolute material-icons text-primaryText top-2 right-2 !text-2xl cursor-pointer" >close</span>
