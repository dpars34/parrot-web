...
const EntryCard: React.FC<IEntryCardProps> = ({ item }) => {
  return (
    <div className="bg-white p-6 font-line rounded-lg text-primaryText"> 
      <h2 className="font-bold text-2xl	h-[4.5rem]">{ item.phrase_title }</h2>
      <div className="font-bold h-12 text-secondaryText">{ item.phrase_standard }</div>
      <div className="text-sm h-12 text-secondaryText">{ item.phrase_explain }</div>
